"""Shared chronological calibration for binary sports markets.

The ``home_*`` field names are retained for compatibility with the original NBA
implementation.  For tennis, "home" means the fixture's first listed player;
the opposite side is always derived as the complement.
"""
...
class CalibrationObservation:
    """One independent binary-outcome calibration row."""
...
class CalibrationPrediction:
    """One point-in-time out-of-sample calibration prediction."""
...
class BootstrapCalibration:
    """Point calibrator plus block-bootstrap models for a lower bound."""
    ...
    def probabilities_for_side(
        ...
        *,
        ...
        """Return point-calibrated and conservative values for one side."""
...
def fit_bootstrap_calibration(
    ...
    *,
    ...
    point_model = fit_logistic_calibrator(ordered, regularization=regularization)
    ...
    for _ in range(bootstrap_samples):
        ...
        models.append(fit_logistic_calibrator(sample, regularization=regularization))
